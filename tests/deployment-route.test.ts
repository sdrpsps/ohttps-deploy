...
async function run() {
  ...
  const response = await POST(new Request("http://localhost/api/deployments", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ certificateId: "certificate-1", certificateVersionId: "version-1" }) }));
