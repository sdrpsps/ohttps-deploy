...
async function run() {
  ...
  const serverResponse = await patchServer(new Request("http://localhost/api/servers/server-1", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: "New server", host: "new.example.com", port: 2222, username: "deploy", hostFingerprint: "SHA256:test" }) }), { params: Promise.resolve({ id: "server-1" }) });
  assert.equal((await certificateResponse.json()).data.name, "New certificate");
  assert.equal((await serverResponse.json()).data.port, 2222);
