...
export function NotificationPanel() {
  ...
  const { data: rows = [] } = useQuery({ queryKey: queryKeys.notifications, queryFn: () => getApiData<Notification[]>("/api/notifications") });
  async function retry(id: string) { await fetch("/api/notifications", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id }) }); await queryClient.invalidateQueries({ queryKey: queryKeys.notifications }); }
