...
const querySchema = z.object({ deploymentId: z.string().optional(), certificateId: z.string().optional(), serverId: z.string().optional(), from: z.coerce.date().optional(), to: z.coerce.date().optional() });
...
export async function GET(request: Request) {
  ...
  const { deploymentId, certificateId, serverId, from, to } = parsed.data;
  const conditions = [isNull(logs.syncJobId), deploymentId ? eq(logs.deploymentId, deploymentId) : undefined, certificateId ? eq(deployments.certificateId, certificateId) : undefined, serverId ? eq(deploymentTargets.serverId, serverId) : undefined, from ? gte(logs.createdAt, from) : undefined, to ? lte(logs.createdAt, to) : undefined].filter((condition): condition is SQL => Boolean(condition));
