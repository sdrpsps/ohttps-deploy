...
echo -e "${BLUE}${BOLD}==========================================================${RESET}"
echo "在 ohttps-deploy 网页控制台中添加该服务器时，建议配置："
echo -e "  • 用户名 (Username):        ${BOLD}${DEPLOY_USER}${RESET}"
echo -e "  • 证书存放目录 (Cert Dir):   ${BOLD}${CERT_DIR}/<domain>/${RESET}"
echo -e "  • 证书文件路径格式:         ${BOLD}${CERT_DIR}/<domain>/fullchain.pem${RESET}"
echo -e "  • 私钥文件路径格式:         ${BOLD}${CERT_DIR}/<domain>/privkey.pem${RESET}"
if [[ -n "$DOCKER_CONTAINER" ]]; then
